...
- 📍 **对话展开位置稳定** - 展开推理、工具和 SubAgent 详情时保持目标事件位置，避免详情加载或内容高度变化导致滚动漂移
- 🖼️ **多模态资源路径支持** - `view_*` 资源工具支持工作区相对路径、普通绝对路径及 `file://`、`jar://`、`jrt://` 等 IntelliJ VFS 协议路径，图片等资源可正确读取并交给资源 Agent 分析
- 🧹 **停止读取全局比例** - 保留历史全局配置数据，但运行时和全局设置页不再读取或展示 `message.history_token_ratio`
- ⏹️ **模型回复停止响应修复** - 停止操作优先取消内存中的活动模型或工具批次，避免任务状态查询被数据库全局写锁阻塞；任务已完成或状态不明确时自动刷新任务列表
- ✅ **验证** - 完成插件构建、目标 Kotlin 测试和 Agent Web 前端构建

### v1.1.5.6 (上一版本)
- 🛡️ **编码会话配置严格校验** - 项目会话必须包含 `project_path`，并要求其规范化路径与 `cwd` 一致；缺少项目路径时不再静默回退，配置错误直接暴露
- 🔒 **项目会话隔离** - 会话查询统一限定为 `coding` 类型且 `visibility=project` 的项目会话，避免非项目会话被编码会话 CRUD 误读或修改
- 📂 **会话可见性路径规范化** - 会话列表按规范化后的项目路径过滤，确保会话只在所属项目中显示，并正确处理等价的路径表示
- 🧭 **会话操作边界统一** - 会话获取、更新和删除复用同一套项目会话约束，保持会话管理与项目可见性规则一致
- 📦 **版本元数据同步** - 插件版本号升级到 `v1.1.5.6`，并同步 SDK Helper 版本元数据到 `1156`

### v1.1.5.5
- ♻️ **编码会话重写** - 对话链路按 awake-claw 编码会话重建。消息事件、任务、追加、附件和上下文压缩替换旧消息卡队列。这是大版本切断：不迁移、不兼容、不降级。升级前请删除 `~/.jtools/agent/agent.db`
- 🧩 **编码环境能力包** - 会话绑定编码环境，不再把 Agent 当模型真源。系统提示词、内置工具、插件函数、技能、多模态 `view_*` 和压缩 Agent 都由环境提供。建会话必须先有环境
- 🎛️ **输入框与布局对齐** - 模型、推理、附件、文件上下文放在输入框左下角；会话入口留在顶栏，其余管理入口收进右上角二级菜单。高级参数与模型管理对齐，附加 JSON 参数直接铺开。上下文指标出现在输入框下方
- 💬 **时间线布局** - 对话框全宽。模型回复靠左 95%；用户气泡靠右，随内容撑开，最大 95% 后软换行。图标改用 Element Plus 并适配亮暗主题，叠在高亮竖线上
- 📝 **Markdown 与代码块** - 回复使用 CodeMirror/Lezer 高亮，HTML 代码块可预览。下载走 IDE 保存对话框，并按 Windows/macOS/Linux 清洗文件名
- 🛠️ **运行时工具** - 插件函数按 Agent 管理页同一套方式注册。`web_fetch` 与 bash 输出上限读取全局设置。MCP 仍走 `cli`。蒸馏 UI 已移除，只保留压缩
- 🗜️ **异步上下文压缩** - 压缩在后台执行，成功或失败都以时间线通知呈现。未出通知前不能发送；模型处理中或排队中不能压缩
- ⚡ **流式性能** - 增量只补当前事件，不再每次整包替换宿主状态。对话滚动按 awake-claw 贴底跟随，用户上翻后停止跟随
- 📎 **附件** - 纯文本模型会收到文件名、类型、大小和路径等元数据。时间线、处理中和排队卡片可预览图片。文件上下文只发送路径和行号
- 🐛 **对话框稳定性** - 流式回复和推理写入事件；刷新不再丢历史；压缩/发送/保存离开 EDT；空库能打开对话框；全局会话工作区为 `~/.jtools/global`
- 📚 **技能创建与导入** - 新增技能需要名称和描述。导入技能会从所选目录递归发现包含 `SKILL.md` 的技能，并复制到 `~/.jtools/skills`
- 📦 **版本元数据同步** - 插件版本号升级到 `v1.1.5.4`，并同步 SDK Helper 版本元数据到 `1154`
