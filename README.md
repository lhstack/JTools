...
### v1.1.5.5 (当前版本)
- 🧾 **Agent 事件持久化** - 新增 `agent_events` 独立事件存储，按 Agent 记录用户消息、推理、工具调用/结果、模型回复和失败事件
- 🧠 **Agent 历史统一** - Agent 直接执行按 Agent 维度恢复历史，跨会话共享上下文；资源查看、上下文压缩和 SubAgent 等隔离任务不写入普通历史
- 📐 **上下文预算控制** - 编码会话按模型上下文窗口、输出预留、历史上限和工具调用保留轮次筛选历史，避免历史过长挤占当前请求
- 🔗 **插件函数会话上下文** - SDK 插件函数支持接收当前会话名称、会话 ID、供应商和模型信息，插件可以准确识别调用上下文
- ⚙️ **模型执行参数补全** - 接入最大工具调用轮次和工具调用历史保留策略，编码会话与通用 Agent 执行共用运行时约束
- 📦 **版本元数据同步** - 插件版本号升级到 `v1.1.5.5`，并同步 SDK Helper 版本元数据到 `1155`
