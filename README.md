...
### v1.1.4.8 (当前版本)
- 📎 **文件上下文开关** - 对话输入区新增文件上下文能力，开启后可将当前编辑器打开文件路径附加到用户消息开头
- 🎯 **选区 offset 上下文** - 有选区时附加 `startOffset/endOffset`；UI 以短 chip 展示（如 `...Server.java 962,1470`），悬停显示完整路径与详情
- 🗂️ **项目维度开关** - 文件上下文开关按项目路径隔离持久化，不同项目互不影响
- 🔧 **read_project_files 支持 offset** - 读取工具新增 `offset_ranges`，可直接按编辑器选区 offset 读取内容（与 `line_ranges` 二选一）
- 🧵 **编辑器 read-action 修复** - 同步文件上下文时在 read-action 内访问选区/文档，避免 EDT Timer 线程异常
- 🎨 **图标与交互** - 新增文件上下文图标按钮，chip 支持关闭；切换文件/选区实时更新
- 📦 **版本元数据同步** - 插件版本号升级到 `v1.1.4.8`，并同步 SDK Helper 版本元数据到 `1148`
