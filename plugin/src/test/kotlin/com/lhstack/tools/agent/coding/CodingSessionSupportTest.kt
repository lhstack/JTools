...
class CodingSessionSupportTest {
    ...
    @Test
    fun `applyReasoningOverride writes max into completions params`() {
        val snapshot = JsonObject().apply {
            addProperty("api", "completions")
        ...
        assertNull(params.get("reasoning"))
        assertNull(params.get("output_config"))
    }

    @Test
    fun `applyReasoningOverride writes max into responses params without reasoning_effort`() {
        val snapshot = JsonObject().apply {
            addProperty("api", "responses")
            add("model_params", JsonObject().apply { addProperty("reasoning_effort", "high") })
        }
        val updated = CodingSessionSupport.applyReasoningOverride(snapshot, "max", null, "openai")
        val params = updated.getAsJsonObject("model_params")
        assertNull(params.get("reasoning_effort"))
