...
class UsageTest {
    ...
    @Test
    fun `drops attribution on parse and accumulate`() {
        val parsed = Usage.fromJson(JsonParser.parseString("""{
          "input_tokens": 12,
          "attribution": [{"entity": "provider", "cost": 1}]
        }"""))
        kotlin.test.assertNull(parsed.toJson().get("attribution"))

        val total = Usage.fromJson(JsonParser.parseString("""{
          "input_tokens": 12,
          "attribution": [{"entity": "old"}]
        }"""))
        total.add(Usage.fromJson(JsonParser.parseString("""{
          "input_tokens": 3,
          "attribution": [{"entity": "new", "tokens": [1, 2, 3]}]
        }""")))
        val json = total.toJson()
        assertEquals(15, json["input_tokens"].asLong)
        kotlin.test.assertNull(json.get("attribution"))
    }
}
