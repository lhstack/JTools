        ...
        tabsPopupGroup.add(object : AnAction({ "在新窗口中打开" }, Icons.newTabIcon()) {
            override fun actionPerformed(e: AnActionEvent) {
                ...
                if (component is TabLabel) {
                    ...
                    // 关闭浮窗时恢复标签显示；FloatingDialog(JFrame) 会在窗口关闭时 dispose 该 disposable。
                    val restoreDisposable = Disposable { tabInfo.isHidden = false }
                    val dialog: FloatingDialog = if (tabInfo.component is PluginTabPanel) {
                        ...
                        FloatingDialog(project, "新窗口", tabInfo.component, restoreDisposable)
                    }
