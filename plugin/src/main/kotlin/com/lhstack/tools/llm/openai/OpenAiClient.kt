...
import com.lhstack.tools.llm.provider.toUserMessage
...
class OpenAiClient(private val params: OpenAiClientParams) {
    ...
    internal fun responses(
        ...
        while (true) {
            ...
            confirmContinuationDelivery(continuation, awaitingDelivery)
            val continued = continueResponsesToolLoop(request, total, round, rounds, toolRuntime, cancel, continuation, awaitingDelivery)
            if (!continued.continueLoop && !cancel.isCancelledOrFalse() && continueWithPendingAppend(request::appendMessages, total, continuation, awaitingDelivery)) {
                continue
            }
            if (!continued.continueLoop) return total
    ...
    private fun chatStreamOnce(url: String, body: JsonObject, cancel: ModelCancel?): ProviderRound {
        ...
        parser.use {
            while (true) {
                ...
                val delta = OpenAiParser.primaryChatDelta(value)
                val hadReasoningDelta = delta?.let { OpenAiParser.chatReasoningDelta(it) }.isNullOrEmpty().not()
                if (delta != null) {
                    collectChatDelta(delta, responseText, reasoning, calls)
                }
                if (!hadReasoningDelta) {
                    OpenAiParser.streamReasoningValue(value)?.let { text ->
                        OpenAiParser.appendReasoningDelta(reasoning, text)
                        streamSink.onReasoningDelta(text)
                    }
                }
    ...
    private fun collectChatDelta(
        ...
        OpenAiParser.chatReasoningDelta(delta)?.let {
    ...
    private fun continueChatToolLoop(
        ...
        if (toolCalls.isEmpty()) {
            params.eventSink?.onStreamEvent("model_round_completed", com.google.gson.JsonObject())
            return LoopResult(false, rounds)
        }
        if (cancel.isCancelledOrFalse()) return LoopResult(false, rounds)
        ...
        claimed?.let { emitClaimedEvents(it) }
    ...
    private fun continueWithPendingAppend(
        ...
        emitClaimedEvents(batch)
        appendMessages(listOf(total.providerMessages.last(), batch.message))
    ...
    private fun emitClaimedEvents(batch: ClaimedContinuationBatch) {
        ...
        private fun roundFromParts(
            ...
            val providerCalls = calls.values.map { call ->
                ProviderToolCall(
                    ...
                    callId = call.id,
                    name = call.name,
                    arguments = com.lhstack.tools.llm.LlmJson.parseArgs(call.arguments.toString()),
                )
            }
...
private fun ModelCancel?.isCancelledOrFalse(): Boolean = this?.isCancelled() == true
