...
internal object OpenAiParser {
    ...
    private fun chatReasoning(message: JsonObject): List<String> {
        val text = (message.getString("reasoning_content")
            ?: message.getString("reasoning")
            ?: message.getString("thinking"))
            ?.trim()
    ...
    fun primaryChatDelta(value: JsonElement): JsonObject? {
        val choices = value.obj()?.getArray("choices") ?: return null
        for (choice in choices) {
            val obj = choice.obj() ?: continue
            val index = obj.get("index")?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isNumber }?.asInt
                ?: throw IllegalStateException("OpenAI stream choice missing index")
            if (index == 0) return obj.getObject("delta")
        }
        return null
    }

    fun chatReasoningDelta(delta: JsonObject): String? =
        delta.getString("reasoning_content")
            ?: delta.getString("reasoning_text")
            ?: delta.getString("reasoning")
            ?: delta.getString("thinking")

    fun streamReasoningValue(value: JsonElement): String? {
        val obj = value.obj() ?: return null
        val message = obj.getArray("choices")?.firstOrNull()?.obj()?.getObject("message")
        return sequenceOf(
            obj.getString("reasoning_content"),
            obj.getString("reasoning_text"),
            obj.getString("reasoning"),
            message?.getString("reasoning_content"),
            message?.getString("reasoning_text"),
            message?.getString("reasoning"),
        ...
        when (obj.getString("type").orEmpty()) {
            ...
            "function_call" -> calls.add(
                ProviderToolCall(
                    ...
                    arguments = com.lhstack.tools.llm.LlmJson.parseArgs(obj.getString("arguments") ?: "{}"),
                )
            )
        }
    }
    ...
    private fun chatToolCall(value: JsonElement): ProviderToolCall {
        ...
        return ProviderToolCall(
            ...
            arguments = com.lhstack.tools.llm.LlmJson.parseArgs(function.getString("arguments") ?: "{}"),
        )
    }
