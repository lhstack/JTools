...
import com.lhstack.tools.llm.provider.AppendMessage
...
class AnthropicClient(private val params: AnthropicClientParams) {
    ...
    internal fun chat(
        ...
        while (true) {
            ...
            val continued = continueToolLoop(request, total, round, rounds, toolRuntime, cancel, continuation, awaitingDelivery)
            if (!continued.continueLoop && cancel?.isCancelled() != true && continueWithPendingAppend(request, total, continuation, awaitingDelivery)) {
                continue
            }
            if (!continued.continueLoop) return total
    ...
    private fun messageStreamOnce(body: JsonObject, anthropicBeta: String?, cancel: ModelCancel?): ProviderRound {
        ...
        parser.use {
            while (true) {
                ...
                when (anthropicStreamEventName(event.event, obj)) {
                    "message_start" -> {
    ...
    private fun openStream(url: String, body: JsonObject, anthropicBeta: String?, cancel: ModelCancel?): SseParser {
        ...
        httpTrace?.request(url, serialized)
        return try {
            executor.sendJsonStream(url, messageHeaders(anthropicBeta), serialized.toString(), cancel, params.firstTokenTrace)
        } catch (e: ModelHttpStatusException) {
            httpTrace?.error(url, e.status.toString(), "model API returned non-success status", JsonPrimitive(e.bodyText))
            throw IllegalStateException("model API returned ${e.status}: ${e.bodyText}")
        } catch (e: ModelRequestCancelledException) {
    ...
    private fun anthropicStreamEventName(sseEventName: String, value: JsonObject): String {
        val eventName = sseEventName.trim()
        if (eventName.isNotEmpty() && eventName != "message") return eventName
        return value.get("type")?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isString }?.asString
            ?: eventName
    }
    ...
    private fun continueToolLoop(
        ...
        toolRuntime: ToolRuntime,
        cancel: ModelCancel?,
        continuation: ModelContinuationPort,
        awaitingDelivery: MutableList<String>,
    ): LoopResult {
        val toolCalls = mergeProviderRound(total, round)
        if (toolCalls.isEmpty()) {
            params.eventSink?.onStreamEvent("model_round_completed", JsonObject())
            return LoopResult(false, rounds)
        }
        if (cancel?.isCancelled() == true) return LoopResult(false, rounds)
        ...
        val claimed = continuation.claimPending()
        val appended = buildList {
            total.providerMessages.lastOrNull()?.let(::add)
            add(toolMessage)
            claimed?.let { add(it.message) }
        }
        claimed?.claimedEvents?.forEach { params.eventSink?.onStreamEvent(it.eventType, it.data) }
        request.appendMessages(appended)
        total.providerMessages.add(toolMessage)
        total.historyMessages.add(toolMessage)
        if (claimed != null) {
            total.providerMessages.add(claimed.message)
            total.historyMessages.add(claimed.historyMessage)
            awaitingDelivery.add(claimed.batchId)
        }
        return LoopResult(true, nextRounds)
    }

    private fun continueWithPendingAppend(
        request: AnthropicMessageRequest,
        total: ProviderRound,
        continuation: ModelContinuationPort,
        awaitingDelivery: MutableList<String>,
    ): Boolean {
        val batch = continuation.claimOrSeal() ?: return false
        batch.claimedEvents.forEach { params.eventSink?.onStreamEvent(it.eventType, it.data) }
        request.appendMessages(listOf(total.providerMessages.last(), batch.message))
        total.providerMessages.add(batch.message)
        total.historyMessages.add(batch.historyMessage)
        awaitingDelivery.add(batch.batchId)
        return true
    }

    private fun confirmContinuationDelivery(continuation: ModelContinuationPort, awaitingDelivery: MutableList<String>) {
        awaitingDelivery.toList().forEach { continuation.markDelivered(it) }
    ...
    companion object {
        ...
        private fun mergeProviderRound(total: ProviderRound, round: ProviderRound): List<ProviderToolCall> {
            ...
            total.historyMessages.addAll(round.providerMessages)
        ...
        private fun toolResultMessage(results: List<com.lhstack.tools.llm.UserContent>): Message {
            if (results.isEmpty()) {
...
private class StreamBlock(
    ...
    fun intoToolCall(): ProviderToolCall {
        val arguments = if (partialInput.isEmpty()) {
            ...
            com.lhstack.tools.llm.LlmJson.parseArgs(partialInput.toString())
        }
