...
import com.lhstack.tools.llm.ImageDetail
...
internal object OpenAiMessages {
    ...
    private fun chatContentPart(content: UserContent): JsonObject = when (content) {
        ...
        is UserContent.Image -> JsonObject().apply {
            ...
            add("image_url", JsonObject().apply {
                addProperty("url", LlmJson.imageUrl(content.image))
                addProperty(
                    "detail",
                    LlmJson.imageDetail(content.image.detail ?: ImageDetail.Auto),
                )
            })
        }
        ...
        is UserContent.Audio -> JsonObject().apply {
            ...
            add("input_audio", JsonObject().apply {
                addProperty("data", LlmJson.sourceData(content.audio.data))
        ...
        is UserContent.Document -> JsonObject().apply {
            ...
            add("file", JsonObject().apply {
                addProperty("file_data", LlmJson.sourceData(content.document.data))
        ...
        is UserContent.Video -> JsonObject().apply {
            ...
            add("input_video", JsonObject().apply {
                addProperty("data", LlmJson.sourceData(content.video.data))
    ...
    private fun responseContentPart(content: UserContent): JsonObject = when (content) {
        ...
        is UserContent.Image -> JsonObject().apply {
            ...
            addProperty("image_url", LlmJson.imageUrl(content.image))
            addProperty(
                "detail",
                LlmJson.imageDetail(content.image.detail ?: ImageDetail.Auto),
            )
    ...
    fun formatToolResultContent(content: List<com.lhstack.tools.llm.ToolResultContent>): String {
        return content.joinToString("\n") { item ->
            when (item) {
                is com.lhstack.tools.llm.ToolResultContent.Text -> item.text
                is com.lhstack.tools.llm.ToolResultContent.Image ->
