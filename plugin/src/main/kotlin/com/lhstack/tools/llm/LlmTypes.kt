...
sealed class AssistantContent {
    ...
    data class Reasoning(val reasoning: com.lhstack.tools.llm.Reasoning) : AssistantContent()
    data class ToolCall(val toolCall: com.lhstack.tools.llm.ToolCall) : AssistantContent()
...
sealed class UserContent {
    ...
    data class Image(val image: com.lhstack.tools.llm.Image) : UserContent()
    data class Audio(val audio: com.lhstack.tools.llm.Audio) : UserContent()
    data class Video(val video: com.lhstack.tools.llm.Video) : UserContent()
    data class Document(val document: com.lhstack.tools.llm.Document) : UserContent()
    data class ToolResult(val toolResult: com.lhstack.tools.llm.ToolResult) : UserContent()
...
sealed class ToolResultContent {
    ...
    data class Image(val image: com.lhstack.tools.llm.Image) : ToolResultContent()
...
class Usage private constructor(
    ...
    companion object {
        fun withoutAttribution(value: JsonElement): JsonElement {
            if (!value.isJsonObject) return value
            val copy = value.asJsonObject.deepCopy()
            copy.remove("attribution")
            return copy
        }

        fun fromJson(value: JsonElement): Usage {
            val fields = value.takeIf { it.isJsonObject }?.asJsonObject?.deepCopy() ?: JsonObject()
        ...
        private fun mergeUsageSnapshot(target: JsonObject, source: JsonObject) {
            for ((key, incoming) in source.entrySet()) {
                if (key == "attribution" || incoming.isJsonNull) continue
                val existing = target.get(key)
...
class ProviderRound(
    ...
    val historyMessages: MutableList<Message> = mutableListOf(),
    val appendMessages: MutableList<com.lhstack.tools.llm.provider.InjectedAppendMessage> = mutableListOf(),
