...
object MessageStoreService {
    ...
    fun clearSession(sessionId: Long) = AgentDatabase.execute { session ->
        ...
        deleteMessageEventGraph(session, sessionId)
    ...
    fun deleteMessageTurn(sessionId: Long, turnId: String): Boolean = AgentDatabase.execute { session ->
        ...
        deleteMessageEventGraph(session, sessionId, turnId)
        true
    }

    private fun deleteMessageEventGraph(session: org.apache.ibatis.session.SqlSession, sessionId: Long, turnId: String? = null) {
        val sql = if (turnId == null) {
            """
            delete from message_events
            where session_id=?
              and id not in (
                select parent_event_id from (
                  select parent_event_id from message_events
                  where session_id=? and parent_event_id is not null
                )
              )
            """.trimIndent()
        } else {
            ...
            where session_id=?
              ...
              and id not in (
                select parent_event_id from (
                  select parent_event_id from message_events
                  where session_id=? and turn_id=? and parent_event_id is not null
                )
              )
            """.trimIndent()
        }
        session.connection.prepareStatement(sql).use { statement ->
            while (true) {
                ...
                if (turnId == null) {
                    ...
                    statement.setString(4, turnId)
                }
                if (statement.executeUpdate() == 0) break
            }
        }
        val remaining = session.getMapper(MessageEventMapper::class.java).selectCount(
            QueryWrapper<MessageEventEntity>().eq("session_id", sessionId).apply {
                if (turnId != null) eq("turn_id", turnId)
            },
        )
        if (remaining > 0) {
            throw IllegalStateException("清空消息事件失败：仍有 $remaining 条事件因 parent_event_id 外键无法删除")
        }
    }
