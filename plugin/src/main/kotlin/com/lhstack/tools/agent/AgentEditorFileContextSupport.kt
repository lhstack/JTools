...
object AgentEditorFileContextSupport {
    ...
    /**
     * 收集编辑器内全部选区：多光标 carets + 列块选（block selection）。
     * 仅用 selectionStart/End 只会拿到主 caret 一处。
     */
    ...
    fun prependToPrompt(prompt: String, snapshot: Snapshot?): String = prependToPrompt(prompt, listOfNotNull(snapshot))
    ...
    private fun resolveVirtualFile(manager: FileEditorManager, editor: Editor?): VirtualFile? {
        if (editor != null) FileDocumentManager.getInstance().getFile(editor.document)?.let { return it }
