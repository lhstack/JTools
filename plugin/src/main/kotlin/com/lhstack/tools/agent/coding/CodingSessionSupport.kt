...
object CodingSessionSupport {
    ...
    fun applyReasoningOverride(snapshot: JsonObject, reasoningLevel: String?, reasoningConfig: JsonObject?, providerKind: String): JsonObject {
        ...
        val api = copy.get("api")?.takeIf { it.isJsonPrimitive }?.asString.orEmpty()
        val anthropic = providerKind.equals("anthropic", ignoreCase = true)
        val responses = api.equals("responses", ignoreCase = true)
        val compatible = copy.get("openai_compatible")?.takeIf { it.isJsonPrimitive }?.asBoolean == true
            || copy.get("compatible")?.takeIf { it.isJsonPrimitive }?.asBoolean == true
        if (level != null) {
            params.remove("reasoning_effort")
            params.remove("reasoning")
            if (!anthropic && !compatible) params.remove("output_config")
        ...
        if (anthropic && reasoningConfig != null && reasoningConfig.isJsonObject) {
