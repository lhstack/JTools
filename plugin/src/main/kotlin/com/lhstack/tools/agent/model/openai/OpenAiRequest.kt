...
internal class OpenAiChatRequest(
    ...
    companion object {
        ...
        fun fromRuntime(
            ...
            return OpenAiChatRequest(body, optionsMaxRetries ?: maxRetries, maxToolRounds)
        }
    }
}
...
internal class OpenAiResponsesRequest(
    ...
    companion object {
        fun fromRuntime(
            ...
            return OpenAiResponsesRequest(body, optionsMaxRetries ?: maxRetries, maxToolRounds)
        }
    }
}
...
private fun optionsFromParams(
    ...
    val modelMap = ModelHttpSupport.objectFromNamedParams(modelParams, "model_params")
    removeNullRequestParams(modelMap)
    removeCompatibleOnlyModelParams(modelMap, openaiProviderType)
    val additionalMap = ModelHttpSupport.objectFromAdditionalParams(additionalParams)
    removeNullRequestParams(additionalMap)
    for ((key, value) in additionalMap.entrySet()) {
        modelMap.add(key, value)
    }
    controlledKeys.forEach { modelMap.remove(it) }
    return modelMap
}
