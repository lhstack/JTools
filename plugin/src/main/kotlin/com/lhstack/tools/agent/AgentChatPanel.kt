...
class AgentChatPanel(private val project: Project) : SimpleToolWindowPanel(true, true), com.intellij.openapi.Disposable {
    ...
    private fun openManagementWindowAfterBrowserCommand(page: String) {
        // JBCefJSQuery waits synchronously for this command to return. Creating another JCEF browser
        // before the response completes deadlocks the CEF query thread and Swing EDT on IntelliJ 2022.3.
        ApplicationManager.getApplication().invokeLater {
            if (!project.isDisposed && !Disposer.isDisposed(this)) managementWindows.open(page)
        }
    }
    ...
    private fun updateToolbars() = actionToolbars.forEach { it.updateActionsImmediately() }
    ...
    private fun handleBrowserCommand(command: AgentBrowserCommand): Any? {
        ...
        return when (command.type) {
            ...
            "window.open" -> openManagementWindowAfterBrowserCommand(text ?: error("缺少管理页面"))
    ...
    /** 润色任务 ID。前端统一以 taskId 字段传递，缺失时直接报错而不静默返回空串。 */
    private fun JsonObject.polishTaskId(): String =
        get("taskId")?.takeUnless { it.isJsonNull }?.asString?.trim()?.takeIf { it.isNotEmpty() }
            ?: error("缺少润色任务 ID")
