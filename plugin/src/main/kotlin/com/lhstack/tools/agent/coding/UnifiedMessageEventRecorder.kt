...
class UnifiedMessageEventRecorder(
    ...
    fun streamSink(turnId: String): ToolEventSink = object : ToolEventSink {
        override fun onToolCall(call: com.lhstack.tools.llm.ProviderToolCall) {
            recordStreamEvent(turnId, "tool_call", JsonObject().apply {
                ...
                addProperty("tool_call_id", call.callId ?: call.id)
        ...
        override fun onToolResult(result: com.lhstack.tools.llm.ToolResult) {
            ...
            recordStreamEvent(turnId, "tool_result", JsonObject().apply {
                ...
                addProperty("internal_call_id", result.id)
    ...
    companion object {
        fun startSubagentEvent(
        ...
        ): UnifiedMessageEventRecorder {
            val parentEventId = MessageStoreService.upsertMessageEvent(
                NewMessageEvent(
                    parentEventId = null,
                    sessionId = sessionId,
                    turnId = turnId,
                    status = MessageEventStatus.RUNNING,
                    eventType = MessageEventType.SUBAGENT,
                    eventId = runId,
                    summary = "subagent · $agentName · $title",
            ...
            val recorder = UnifiedMessageEventRecorder(sessionId, parentEventId, broadcaster)
            recorder.broadcastEventId(parentEventId)
            return recorder
        }
    }

    fun finishSubagentEvent(turnId: String, status: MessageEventStatus, result: JsonObject, error: String? = null) {
        val parentId = parentEventId ?: throw IllegalStateException("SubAgent 事件缺少父记录")
        val existing = MessageStoreService.messageEvent(parentId, true)
            ?: throw IllegalStateException("SubAgent 事件不存在")
        val context = existing.context?.deepCopy() ?: JsonObject()
        context.add("result", result)
        if (error != null) context.addProperty("error", error)
    ...
    fun recordTurnUsage(turnId: String, usage: JsonElement) {
        ...
        context.add("usage", com.lhstack.tools.llm.Usage.withoutAttribution(usage))
        MessageStoreService.upsertMessageEvent(
            NewMessageEvent(
    ...
    private fun recordToolCall(turnId: String, data: JsonObject) {
        val callId = stringField(data, "call_id", "tool_call_id", "internal_call_id", "id")
        ...
        val id = MessageStoreService.upsertMessageEvent(
            NewMessageEvent(
                ...
                eventId = callId,
                summary = stringField(data, "tool_name", "name") ?: "tool",
    ...
    private fun stringField(data: JsonObject, vararg keys: String): String? {
        for (key in keys) {
            val value = data.get(key)?.takeIf { it.isJsonPrimitive }?.asString?.takeIf { it.isNotBlank() }
