...
function scrollMessagesToBottom() {
  programmaticConversationScroll = true
  applyConversationBottom()
  nextTick(() => {
    applyConversationBottom()
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        applyConversationBottom()
        programmaticConversationScroll = false
      })
    })
  })
}
...
function modelCatalogSnapshot() {
  ...
  return {
    ...
    max_history_messages: currentSession.value?.max_history_rounds ?? null,
...
function snapshotSource() {
  ...
  const sameModel = Number(snapshot.id) === Number(selectedModelId.value)
    && Number(snapshot.provider_id) === Number(selectedProviderId.value)
  if (sameModel) return snapshot
  return modelCatalogSnapshot() || snapshot
}
...
async function persistModelSettings({ snapshot = null } = {}) {
  ...
  const settings = {
    ...
    model_snapshot: snapshot || (modelChanged ? modelCatalogSnapshot() : currentSession.value.model_snapshot),
...
async function clear() {
  ...
  try {
    await ElMessageBox.confirm('确定清空当前会话的全部对话记录？', '清空会话', { type: 'warning' })
    await api('session.clear')
  } catch (error) {
    if (error === 'cancel' || error === 'close') return
    ElMessage.error(error?.message || String(error) || '清空失败')
  }
}
  ...
  >
    ...
    <section ref="messageList" class="message-list" @scroll="handleConversationScroll">
      ...
      <ConversationTimeline
        ...
        @layout-changed="onTimelineLayoutChanged"
      />
    </section>
