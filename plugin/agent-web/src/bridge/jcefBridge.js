...
export const hostState=reactive({dark:false,theme:{},sessions:[],agents:[],messages:initialChat.messages,events:[],tasks:[],historyRevision:0,historyBeforeId:null,historyHasMore:false,queue:[],drafts:[],inputRestore:null,currentSessionId:initialChat.sessionId,currentAgentId:null,currentSession:null,providers:[],prompts:[],context:null,skills:[]})
let bridgeReady
...
function installPopupStateBridge(){let reported=null,scheduled=false;const isVisible=element=>{const style=getComputedStyle(element);return element.getClientRects().length>0&&style.display!=='none'&&style.visibility!=='hidden'};const hasOpenPopup=()=>[...document.querySelectorAll('.el-overlay')].some(overlay=>isVisible(overlay)&&overlay.querySelector('.el-dialog,.el-drawer,.el-message-box'))||[...document.querySelectorAll('.el-popper')].some(isVisible);const report=()=>{scheduled=false;const open=hasOpenPopup();if(open===reported)return;reported=open;invoke('ui.popupState',{open}).catch(()=>{})};const schedule=()=>{if(scheduled)return;scheduled=true;queueMicrotask(report)};new MutationObserver(schedule).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']});report();setInterval(()=>{document.querySelectorAll('.el-overlay').forEach(overlay=>{if(isVisible(overlay)&&!overlay.querySelector('.el-dialog,.el-drawer,.el-message-box')) overlay.remove()});report()},1500)}
